    ...
    <ScrollView style={styles.container} contentContainerStyle={{ padding: spacing.md }}>
      <Card title="Adjustable Pseudo Quantum Bit">
        <View style={styles.hero}>
          <Mono color={colors.accent} style={{ fontSize: 16, fontWeight: '500' }}>{`|ψ(θ)⟩ = ${c.toFixed(4)}|0⟩ + ${s.toFixed(4)}|1⟩`}</Mono>
        </View>
        <Row style={{ marginTop: spacing.sm }}>
          <Stat label="r = cos 2θ" value={(q.r >= 0 ? '+' : '') + q.r.toFixed(3)} color={colors.accent} />
          <Stat label="η = |sin 2θ|" value={eta.toFixed(3)} color={colors.accent2} />
          <Stat label="P(1)" value={p1.toFixed(3)} />
        </Row>
        <Label>θ = {theta.toFixed(4)} rad ({((theta * 180) / Math.PI).toFixed(1)}°)</Label>
        <Slider minimumValue={0} maximumValue={Math.PI / 2} value={theta} onValueChange={setTheta} minimumTrackTintColor={colors.accent} maximumTrackTintColor={colors.fill} thumbTintColor={colors.accent} />
        <Row>
      ...
      <Card title="r² + η² = 1" accent={colors.accent2}>
      ...
      <Card title="η as a Control Signal (Eq. 11 / 32)" accent={colors.warn}>
      ...
      <Card title="Chebyshev Features (Prop. 2)" accent={colors.teal}>
        {re.map((v, k) => (
...
const styles = StyleSheet.create({
  ...
  hero: { backgroundColor: colors.accentSoft, borderRadius: radius.sm + 2, paddingHorizontal: 12, paddingVertical: 10, alignItems: 'center' },
  track: { flex: 1, height: 12, backgroundColor: colors.fill, borderRadius: 6, overflow: 'hidden' },
  zero: { position: 'absolute', left: '50%', width: 1, height: '100%', backgroundColor: colors.borderStrong },
  fill: { position: 'absolute', height: '100%', borderRadius: 6 },
});
