...
import { Histogram, KV, Label, Mono, ReadoutTable } from './components';
    ...
    <View>
      {info.concurrence !== undefined ? (
        <>
          ...
          <Mono color={colors.dim} style={{ fontSize: 11 }}>{`Eq. 13-14: C₂ = η, C₂² + r² = 1 for |Ψ₂(θ)⟩ — ${Math.abs(info.c2Check! - 1) < 1e-9 ? 'satisfied' : 'outside that family'}`}</Mono>
        </>
      ) : null}
      {info.threeTangle !== undefined ? (
        <>
          ...
          <Mono color={colors.dim} style={{ fontSize: 11 }}>{`Eq. 16-17: τ₃ = η², τ₃ + r² = 1 for |Ψ₃(θ)⟩ — ${Math.abs(info.tau3Check! - 1) < 1e-9 ? 'satisfied' : 'outside that family'}`}</Mono>
        </>
      ) : null}
